...
class Reports extends CI_Controller {
	...
	function __construct(){
      ...
      public function dhourly_report(){  
        ...
        if($interval!='null'){
           $sql.= " interval_time LIKE '%$interval%' AND";
        }

        $query=substr($sql,0,-3);
        $count=$this->super_model->custom_query("SELECT * FROM rtd_info WHERE $query");
        if($count!=0){
            foreach($this->super_model->custom_query("SELECT * FROM rtd_info WHERE $query ORDER BY interval_time, price_node ASC") AS $all){
                $data['info'][] = array(
    ...
    public function export(){
        ...
        if($unit !='null' && $date!='null' && $time!='null'){
            $interval = $date ." ". $time1;
            foreach($this->super_model->custom_query("SELECT * FROM rtd_info WHERE interval_time LIKE '%$interval%' AND price_node = '$unit'") AS $all){
